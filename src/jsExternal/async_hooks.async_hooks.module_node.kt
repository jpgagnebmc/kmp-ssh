...
external open class AsyncResource {
    ...
    open fun <Func : (args: Any) -> Any> bind(fn: Func): Func /* Func & `T$24` */
    ...
    companion object {
        fun <Func : (args: Any) -> Any> bind(fn: Func, type: String = definedExternally): Func /* Func & `T$24` */
    }
}
...
external open class AsyncLocalStorage<T> {
    open fun disable()
    open fun getStore(): T?
    open fun <R> run(store: T, callback: (args: Any) -> R, vararg args: Any): R
    open fun <R> exit(callback: (args: Any) -> R, vararg args: Any): R
